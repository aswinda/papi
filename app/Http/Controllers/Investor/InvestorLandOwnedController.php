...
class InvestorLandOwnedController extends Controller {
	...
	*/
	...
	{
		$land = LandTransaction::detailLandsOwnedWithImage($id);

		return view('investor.landowned.detail')
			->with('land', $land);
	}
	...
	public function listLandOwned($page = 1)
	{
		$user = Auth::user();
		...
		return view('investor.landowned.index')
			->with('lands', $lands)
			->with('page', $page)
			->with('page_count', $page_count);
	}
